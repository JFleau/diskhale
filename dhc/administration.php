...
<div id="formulaire" class="formulaire" style="height:200px;">
    ...
    <form method="post">
        ...
        <input type="submit" value="Annuler" style="margin-top:20px; margin-bottom:20px; margin-right:10px; width:auto"/>
    </form>
        </div>
...
//    else{
   ...
   if($trigramme!=""){
        ...
        <form method="post" onclick="">
            <input type="hidden" value="modifier" name="modif">
            <input type="hidden" value="<?php echo $trigramme;?>" name="trigramme">
...
$string2="SELECT * FROM `emprunts` WHERE `trigramme`='$trigramme' AND `daterendu`=0000-00-00 ";
    ...
    while($tab2 = mysql_fetch_assoc($query2)){
        ...
        while($tab3 = mysql_fetch_assoc($query3)){
            ...
            $rem=$tab3['remarques'];
            if($tab3['oeuvres']!=""){ $ouvre=$ouvre."<br/>";}
