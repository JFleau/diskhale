...
<?php
  ...
  while ($row=mysql_fetch_row($result)) {
    ...
    echo '<tr><td>' ;
    if ($modif==1) {
      echo '<a href="modifier_disque.php3?codelettres='.$row[0].'&&numero='.$row[1].'">&nbsp;M&nbsp;</a><td>' ;
      echo '<a href="script_effacer_disque.php3?codelettres='.$row[0].'&&numero='.$row[1].'">&nbsp;X&nbsp;</a><td>' ;
      echo '<a href="script_historique_disque.php3?codelettres='.$row[0].'&&numero='.$row[1].'">&nbsp;H&nbsp;</a><td>' ;
    }
    echo $row[0].'<td>'.$row[1].'<td>'.$row[2].'<td>'.$row[3].'<td>'.$row[4].'<td>'.$row[5].'<td>'.$row[6] ;
    echo '&nbsp;<td><font color="#FF0000">' ;
    if ($modif==1)
      echo '<a href="traiter_client.php3?trigramme='.$emprunteur.'">'.$emprunteur.'</a>' ;
    else
  ...
  echo "<br><br><br>\n" ;
  echo '<a href="' ;
  if ($modif==1) echo 'chercher_disque.php3?modif=1' ;
  else echo 'chercher_disque.php3' ;
