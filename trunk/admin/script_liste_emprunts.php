...
<?php
  ...
  while ($row=mysql_fetch_row($result)) {
    ...
    echo '<tr><td>' ;
    echo $row[0].'<td>'.$row[1].'<td>'.$row[2].'<td><a href="traiter_client.php?trigramme='.$row[3].'">'.$row[3].'</a><td>'.$row[4] ;
    echo '<td>&nbsp;<a href="script_cherche_disque.php?modif=1&&codelettres='.$row[0].'&&numero='.$row[1].'">' ;
